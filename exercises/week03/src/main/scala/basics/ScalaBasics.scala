...
object ScalaBasics {
  ...
  def isPalindrome(s: String): Boolean = {
    val normal = for(x <- s if x.isLetter) yield x.toLower
  ...
  def wordCounter(lines: Array[String]): Map[String, Int] = {
    var wordMap: Map[String, Int] = Map()
    for (line <- lines; sentence = line.split(" "); word <- sentence) {
      if(wordMap.contains(word)){
        wordMap.updated(word,wordMap.get(word).get+1)
      }
      else wordMap += (word -> 1)
    }
    wordMap
