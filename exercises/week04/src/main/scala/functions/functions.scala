...
object Funcs {
  ...
  def maxAverage(ls: List[(Double, Double)]): Double = {
    val numbers = map(ls)(a => if (a._1 > a._2) a._1 else a._2)
    foldLeft(numbers, 0.0) (_ + _) / foldLeft(numbers,0.0) ((a , b) => 1.0 + a)
  }
  ...
  def variance(ls: List[Double]): Double = {
    ...
    val size = length(ls)
    val mean = total / size
    sum(map(ls) (v => Math.pow(v - mean,2))) / size
  }
}
